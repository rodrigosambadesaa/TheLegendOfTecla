...
try {
    ...
    & mvn '-Denforcer.skip=true' `
        '-Dtest=ComandosCompuestosConsolaGuiTest,ReglasCombateYVictoriaTest,IntegracionModosGuiTest' `
    ...
    $comandos = "ayuda`nmirar`ninventario`ndescansar`nsalir`n"
    $comandos | & java -jar $jarJuego --rapido --nombre Tecla --clase marine `
